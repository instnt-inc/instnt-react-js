...
describe(`Testing Instnt Signup Provider Component For ${formDataObj[0].FORM_KEY}`, ()=>{

  let event_type = ''
  let transaction_decision = ''

  const item = formDataObj[0]
  
  const onEventCallBack = (event) =>{
    event_type = event?.type;
    if(event_type === item.TRANSACTION_PROCESSED){
  ...
  describe('TRANSACTION_INITIATED_EVENT after begin transaction', ()=>{
    ...
    it('should contain otpVerification as false after TRANSACTION_INITIATED', async ()=>{
      ...
      expect(otpVerification).toBe(false)
    })
    
    it('should contain documentVerification as false after TRANSACTION_INITIATED', async ()=>{
      ...
      expect(documentVerification).toBe(false)
    })

    it('should contain SSI as false after TRANSACTION_INITIATED', async ()=>{
